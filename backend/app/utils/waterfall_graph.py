...
def show_waterfall_graph(model,
                    cal_model,
                    x1, 
                    top_k=15):
    """
    model: CatBoost модель
    x1: объект, для которого отрисовываем график
    top_k: количество фичей на графике
    """
    ...
    ax.set_title(f"Waterfall (log-odds)\n f(x) = {final_value:.3f} \n Probability={cal_model.predict_proba(x1)[:, 1][0]:.3f}", fontsize=14)
    ...
    print(f'Predicted calibrated probability: {cal_model.predict_proba(x1)[:, 1][0]}')
